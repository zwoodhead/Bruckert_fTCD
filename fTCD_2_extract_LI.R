...
if (!file.exists(paste0(rootdir,"/",task,'_trial_inclusion.csv'))){
  if (task_switch == 1){
    osf_retrieve_file("https://osf.io/h7n39") %>% osf_download(conflicts = "skip") # WordGen_trial_inclusion.csv
  }
  if (task_switch == 2){
    osf_retrieve_file("https://osf.io/rvcd2") %>% osf_download(conflicts = "skip") # PPTT_trial_inclusion.csv
  }
}
...
for (mysub in (1:length(to_run))){ 
  ...
  results_row <- which(results$Filename == mysubname)
